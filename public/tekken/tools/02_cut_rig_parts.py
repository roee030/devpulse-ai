...
def save_part(rgba, mask, name, pivot, cid, feather=1.0):
    ...
    img.save(f'rig/parts/{cid}_{name}.webp', quality=92, method=6); img.save(f'rig/parts/{cid}_{name}.png')
...
for cid in IDS:
    ...
    torso |= capsule(w,h,hipC,neck, max(abs(P['shL'][0]-P['shR'][0])*0.5, 0.09*w)*0.72)
    ...
    print(cid, 'parts', len(parts), 'sole', RIG[cid]['sole'], flush=True)
